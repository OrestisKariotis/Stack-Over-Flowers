...
public class Elastic {
    ...
    public void add(Event2 event) throws RuntimeException { //edw mpainoun ta event  //THN ALLAKSA SE PUBLIC
        try {
            ...
            writer.beginObject();
            writer.name("lat").value(event.getLocation().getX());
            writer.name("lon").value(event.getLocation().getY());
            writer.endObject();
            writer.name("categories").value(event.getCategories());  //to pedio tags enai pio tricky
...
//          		writer.beginArray();
            ...
            writer.name("ticket_cost").value(event.getTicket_cost());
            // writer.name("age").value(event.getAge());  //to allaksame eipame
            writer.name("lowestAge").value(event.getLowestAge());
            writer.name("highestAge").value(event.getHighestAge());
            writer.name("date").value(event.getDate());
            writer.name("event_id").value(event.getEvent_id());
            writer.name("starting_time").value(event.getStarting_time());
...
//            writer.name("hasTickets").value(event.getTicketsAvailable() > 0);
 ...
 public   void bulkedDelete (List <String> ids) throws IOException {
  ...
  public  List<String> searchIdsToDelee(String today) throws IOException {
    ...
    public List<Map<String, Object>>/*List<String>*/ search(String text,List <String> taggs, Long distanceInKm, Location fromLoc, int from, int count,Long age,Long costu,Long costl,String dateu,String datel) {
        ...
        try {
            ...
            fields.put("description", (float) 1.3);//8a xreiastei meta
            ...
            if (taggs != null) {//ta taggs
                boolQuery.must( //must me term squery prpei na ginei
                        QueryBuilders.termsQuery("categories", taggs).boost((float) 1.4)//to kanw me terms (any of therms match)
            ...
            if (age !=null ) { //to range gia ta age ..h times tou eurous dhmiourgountai gia +2 xronia
                boolQuery.filter(
                        QueryBuilders.rangeQuery("lowestAge").lte(age)//boost 1.3
                ...
                boolQuery.filter(
                        QueryBuilders.rangeQuery("highestAge").gte(age)//boost 1.3
            ...
            if (costu !=null && costl!= null) {
                boolQuery.filter(
                        QueryBuilders.rangeQuery("ticket_cost").gte(costl).lte(costu)
                );
            ...
            List<Map<String,Object>>/*<String>*/ results = new ArrayList<>();
            ...
            //    String json = gson.toJson(hit.getSourceAsMap());
                ...
                //	results.add(map);
                results.add(hit.getSourceAsMap());//json);
