...
public class Elastic {
        		...
        		writer.beginObject();	 //arxizw eisagw keys/values
        			...
        			writer.name("date");	//h hmer/mnia
        				writer.beginObject();
        					...
        					writer.name("format").value("dd-MM-yyyy"); //tp date 8e;ei format
        				writer.endObject();
  ...
  //to age kai to cost 8a ta vrei mono tou (ta kanei long )
    ...
    void add(Event2 event) throws RuntimeException { //edw mpainoun ta event
        try {
            //jsonify the event
            ...
            writer.name("location");//to location pio tricky
                ...
                writer.endObject();
           writer.name("tags");  //to pedio tags enai pio tricky
            ...
            writer.name("age").value(event.getAge()); 
...
//            writer.name("hasTickets").value(event.getTicketsAvailable() > 0);
    ...
    void bulkedDelete (List <String> ids) throws IOException {
 ...
 //   }
    ...
    List<String> searchIdsToDelee(String today) throws IOException {
    	
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery(); //to query pou 8a xtisw
        boolQuery.filter(
    			QueryBuilders.rangeQuery("date").lte(today).format("dd-MM-yyyy")//fi;tro pouranaw sto query
    			);		
  ...
  //dokimastikh search perilamvanei
   //free text search (dec,title) 
    ...
    SearchHit[] search(String text,List<String> taggs, Long distanceInKm, Location fromLoc, int from, int count,Long age,Long costu,Long costl,String dateu,String datel) {
 ...
 //           );
           ...
           boolQuery.minimumShouldMatch(1); //lunei nmz to 8ema ton should pou einai optiomnal
            ...
            if (age !=null ) { //to range gia ta age ..h times tou eurous dhmiourgountai gia +2 xronia
            	boolQuery.filter( 
            			QueryBuilders.rangeQuery("age").gte(age-3).lte(age+3)//boost 1.3
            ...
            if(dateu!=null && datel!= null) {
            	boolQuery.filter(
            			QueryBuilders.rangeQuery("date").gte(datel).lte(dateu).format("dd-MM-yyyy")//to format tou
          ...
          //  builder.from(from).size(count).query(boolQuery); //o builder metaferei to query
            builder.query(boolQuery).from(from).size(count); // edw prepei na mpei kai sortarisma !!!!! kai from kai xount
          ...
          //  List<DocumentField> tits = new ArrayList<>(); //lista me ids
